...
def _clean_reply(text: str) -> str:
    """Убирает markdown-блоки кода, JSON-хвосты и числовые ID из ответа модели."""
    import re
    # Убираем ```json ... ``` и ``` ... ```
    text = re.sub(r"```[a-z]*\n?", "", text)
    ...
    # Убираем упоминания ID (типа "ID:123456789" или "ID=123456789" или "id: 123456789")
    text = re.sub(r"\bID[=:]\s*\d{6,}\b", "", text, flags=re.IGNORECASE)
    text = re.sub(r"\b\d{8,}\b", "", text)  # Удаляем длинные числа (telegram IDs)
...
async def _matchmaking_vision_rank(
    ...
    system = (
        ...
        "- НИКОГДА не упоминай числовые ID пользователей в тексте ответа (они технические)\n\n"
        "ФОРМАТ ОТВЕТА:\n"
        "Напиши 1-2 коротких предложения объяснения (живым языком, с эмодзи). "
        "Называй кандидатов по ИМЕНИ, а не по ID.\n"
